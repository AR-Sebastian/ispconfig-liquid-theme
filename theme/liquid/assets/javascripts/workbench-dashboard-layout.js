(function () {
  ...
  var STORAGE_KEY = 'ispconfig-liquid.dashboard.layout.v1';
  ...
  function moduleTitleStem(value) {
    return moduleDisplayTitle(value)
      .replace(/[.\u2026]+$/, '')
      .trim()
      .toLowerCase();
  }

  function moduleDisplayTitle(value) {
    return String(value || '')
      .replace(/\s+/g, ' ')
      .trim()
      .replace(/^(?:gehe\s+zu|go\s+to|aller\s+[àa]|ir\s+a|vai\s+a)\s+/i, '')
      .trim();
  }
  ...
  function splitModuleDashlet(host) {
    ...
    items.forEach(function (item, index) {
      ...
      var moduleLink = item.querySelector('[data-workbench-module], [data-capp]');
      var sourceTitleCandidates = [
        titleNode ? titleNode.textContent : '',
        moduleLink ? moduleLink.getAttribute('title') : '',
        moduleLink ? moduleLink.getAttribute('aria-label') : ''
      ].map(function (candidate) {
        return String(candidate || '').replace(/\s+/g, ' ').trim();
      }).filter(Boolean).sort(function (left, right) {
        return right.length - left.length;
      });
      var title = sourceTitleCandidates[0] || t('moduleWidget') + ' ' + (index + 1);
      var moduleName = moduleLink ? (moduleLink.getAttribute('data-workbench-module') || moduleLink.getAttribute('data-capp') || '') : '';
      var normalizedModuleName = String(moduleName).trim().toLowerCase();
      var navigationItems = Array.prototype.slice.call(document.querySelectorAll(
        '#main-navigation [data-workbench-module], #main-navigation [data-capp], ' +
        '.wb-app-navigation [data-workbench-module], .wb-app-navigation [data-capp], ' +
        '.wb-mobile-navigation [data-workbench-module], .wb-mobile-navigation [data-capp]'
      )).filter(function (candidate) {
        return candidate !== moduleLink && !item.contains(candidate);
      });
      var matchingNavigationTitles = navigationItems.filter(function (candidate) {
        var candidateModule = candidate.getAttribute('data-workbench-module') || candidate.getAttribute('data-capp') || '';
      ...
      if (title) {
        ...
        matchingNavigationTitles = matchingNavigationTitles.concat(prefixNavigationTitles);
      }
      ...
      title = moduleDisplayTitle(title);

...
1. Download `ispconfig-liquid-theme-1.2.4.tar.gz` from the GitHub release.
...
5. Select **LIQUID** in **System → User Settings → Design**.
6. Clear the browser cache and sign in again.

## Enable LIQUID for the sign-in page

The personal theme selection applies only after sign-in. To use LIQUID for the
...
select the ISPConfig default theme and then remove `themes/liquid`. When using
the operator tool, `rollback` restores the newest managed backup.
